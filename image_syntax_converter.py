...
posts_dir = "/Users/patrickyang/ctf/writeups/bakayang-folder/bakayang/src/content/posts/umcs-prequalifers-2025"

# Directory where your original Obsidian attachments are stored.
attachments_dir = "/Users/patrickyang/CTF/writeups/bakayang-folder/old-version/public/images"
...
def convert_html_and_copy(match):
    """
    This function is called by re.sub for each <img> tag found.
    It converts the HTML to Markdown and copies the associated file.
    """
    original_html_tag = match.group(0)
    src_path = match.group(1) # e.g., "/images/skyfall_1.png"

    # Proceed only if the path is in the expected format
    if not src_path.startswith('/images/'):
        print(f"⚠️  Skipping path with unexpected format: {src_path}")
        return original_html_tag # Return original tag if format is wrong
    ...
    # Find the source image file in the Obsidian attachments folder
    ...
    if image_source_path:
        ...
        try:
            ...
            # If copying fails, don't change the text.
            return original_html_tag
    else:
        # If the image isn't found in attachments, don't change the text
        return original_html_tag

    # --- Step 2: Convert the HTML tag to Markdown ---
    
    # Construct the new Markdown path. Astro will resolve this to the assets folder.
    new_markdown_path = f'./assets/{image_name}'
    markdown_link = f"![]({new_markdown_path})"
    
    print(f"🔄 Converting '{original_html_tag}' to '{markdown_link}'")
...
def process_markdown_files():
    ...
    """
    print(f"🚀 Starting HTML-to-Markdown conversion and file copy in: {posts_dir}")
    
    # Regex to find <img> tags and capture the value of the src attribute.
    img_tag_regex = re.compile(r'<img\s+src="([^"]+)"[^>]*>')
    ...
    for filename in os.listdir(posts_dir):
        if filename.endswith(".md"):
            ...
            try:
                ...
                updated_content = img_tag_regex.sub(convert_html_and_copy, content)
                ...
                if updated_content != content:
                    ...
                    print(f"No relevant <img> tags found in {filename}.")
